...
function renderSmartFusionModal(){
  ...
  state.fusionGroups.forEach(function(group){
    ...
    // Avertissements
    if(hasWarnings){
      h+='<div style="background:#fef3c7;border-radius:6px;padding:6px 8px;font-size:11px;color:#b45309;"><strong>âš ïÂ¸Â Attention:</strong> '+group.warnings.join(', ')+'</div>';
    }
...
function renderTerrainPrel(){
  ...
  if(p.subPrelevements.length>1){
    ...
    for(var t=0;t<p.subPrelevements.length;t++){
      ...
      h+='<button class="tab '+(state.activeSubIndex===t?'active':'')+'" onclick="state.activeSubIndex='+t+';render();">Prél '+(t+1)+(sb.completed?' âœ“':'')+'</button>';
    }
    h+='</div>';
  }
...
function renderSubPrelForm(p,sb,idx){
  ...
  h+='<button class="btn btn-success" onclick="toggleSubComplete('+p.id+','+idx+');">'+(sb.completed?'âœ“ Complété - Modifier':'âœ“ Valider')+'</button></div>';
  return h;
}
