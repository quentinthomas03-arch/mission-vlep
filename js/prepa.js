...
function renderPrepaAutoModal(m){
  ...
  if(d.parsed){
    ...
    p.rows.forEach(function(r){
      ...
      h+='<div style="font-size:11px;color:var(--text-muted);margin-top:3px;">'+r.type+regLabel+' â†’ '+r.gehNums.length+' GEH Ã— '+(r.isReg?'3':'1')+' = '+(r.gehNums.length*(r.isReg?3:1))+' sous-prÃ©l.</div>';
      h+='</div>';
    });
    h+='</div>';
